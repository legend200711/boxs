...
// ── App State ──────────────────────────────────────────────
const App = {
  currentUser: null,   // { uid, name, email, handle, bio, photoURL, badges, ... }
  currentPage: 'feed',
  viewingProfile: null,
  activeConvo: null,
  fbApp:   null,       // firebase.app instance
  auth:    null,       // firebase.auth instance
  db:      null,       // firebase.database instance
  useLocalFallback: true,   // set false once Firebase confirms it loaded
  deferredInstallPrompt: null,
  unreadNotifs: 0,
  unreadMessages: 0,
  _listeners: [],      // active RTDB listeners to clean up on sign-out
};
...
async function handleLogin(e) {
  ...
  try {
    if (!App.useLocalFallback && App.auth) {
      // Firebase Auth sign-in (compat SDK)
      await App.auth.signInWithEmailAndPassword(email, pass);
      // onAuthStateChanged will call setCurrentUser
    } else {
...
async function handleRegister(e) {
  ...
  try {
    if (!App.useLocalFallback && App.auth) {
      ...
      // Check if this is the first ever user (founder)
...
function renderProfile(uid) {
  ...
  const users = LocalDB.get('users') || [];
  const user  = users.find(u => u.uid === uid) || App.currentUser;
  if (!user) return;
  App.viewingProfile = uid;

  const isOwn = App.currentUser?.uid === uid;
  const isFollowing = App.currentUser && (user.followers||[]).includes(App.currentUser.uid);
  const posts = (LocalDB.get('posts') || []).filter(p => p.uid === uid);
    ...
    <div class="card card-flush" style="margin-bottom:1rem;">
      ...
      <div class="profile-info">
        ...
        <div class="profile-stats">
          ...
          <div class="stat"><div class="stat-value">${(user.followers||[]).length}</div><div class="stat-label">Followers</div></div>
          <div class="stat"><div class="stat-value">${(user.following||[]).length}</div><div class="stat-label">Following</div></div>
        </div>
...
function runSearch() {
  ...
  const filter = $('search-page')?.querySelector('.filter-chip.active')?.dataset?.filter || 'all';
  const users  = LocalDB.get('users') || [];
  const posts  = LocalDB.get('posts') || [];
  ...
  if (filter === 'all' || filter === 'people') {
    const matched = users.filter(u => u.name?.toLowerCase().includes(q) || u.handle?.toLowerCase().includes(q));
    matched.forEach(u => results.appendChild(buildUserCard(u)));
  }

  if (filter === 'all' || filter === 'posts') {
    const matched = posts.filter(p => p.body?.toLowerCase().includes(q));
    matched.forEach(p => results.appendChild(buildPostEl(p)));
  }

  if (results.children.length === 0) results.innerHTML = `<p class="text-muted text-center mt-2">No results for "<strong>${esc(q)}</strong>"</p>`;
}
...
function initFirebase() {
  try {
    // firebase-app.js, firebase-auth.js, firebase-database.js loaded
    // via compat CDN scripts in index.html
    ...
    App.auth.onAuthStateChanged(async fbUser => {
      if (fbUser) {
        ...
        const snap = await App.db.ref(`users/${fbUser.uid}`).once('value');
        let profile = snap.val();
        if (!profile) {
          // Profile not yet written (e.g. page refreshed mid-register)
          profile = {
            uid:      fbUser.uid,
...
function init() {
  ...
  // Hook up composer submit button
  const postBtn = $('submit-post-btn');
  if (postBtn) postBtn.addEventListener('click', submitPostWithImage);
